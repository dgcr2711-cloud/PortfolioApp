...
def _render_tabela_ativos(lista_ativos: list[dict]) -> None:
    ...
    for a in lista_ativos:
        ...
        linhas_html.append(
            ...
            f'<td>{tipo_texto}</td>'
            f'<td class="col-num">{cotacao_texto}</td>'
            f'<td class="col-center">{alerta_html}</td>'
            f'<td class="col-num">{preco_teto_texto}</td>'
            f'<td class="col-num">{preco_teto_margem_texto}</td>'
            f'<td class="col-center">{indicacao_html}</td>'
            f'</tr>'
        )

    # Mesmo alinhamento (col-num/col-center, ver ui/styles.py) da tabela de
    # Posições da aba Carteira — 2026-09-04, pedido do Diego pra melhorar a
    # estética: as duas tabelas usam a mesma classe CSS, então ficavam
    # visualmente diferentes se só uma ganhasse o alinhamento novo.
    tabela_html = f"""
    <div class="card-tabela">
    <table class="tabela-carteira">
        <thead><tr><th>Ticker</th><th>Tipo</th><th class="col-num">Cotação</th><th class="col-center">Alerta</th><th class="col-num">Preço Teto</th><th class="col-num">Preço Teto c/ Margem (20%)</th><th class="col-center">Indicação</th></tr></thead>
