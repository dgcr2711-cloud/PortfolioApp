...
def _com_caminhos_isolados(testar, conteudo_firebase: dict | None, conteudo_email: dict | None):
    """
    Redireciona gerador.CAMINHO_CHAVE_FIREBASE e gerador.CAMINHO_CONFIG_EMAIL
    pra arquivos temporários (criando-os só se o conteúdo correspondente
    não for None) durante a chamada de `testar`, e sempre restaura os
    valores originais — mesmo se `testar` lançar.
    """
...
def test_gerar_texto_secrets_sem_nenhum_arquivo_configurado_avisa_e_nao_quebra():
    ...
    _com_caminhos_isolados(testar, conteudo_firebase=None, conteudo_email=None)


def test_gerar_texto_secrets_com_ambos_os_arquivos_monta_as_duas_secoes():
    chave_falsa = {
    ...
    def testar():
        ...
        assert "Nenhuma" not in texto  # nenhum aviso de arquivo faltando, já que os dois existem

    _com_caminhos_isolados(testar, conteudo_firebase=chave_falsa, conteudo_email=email_falso)


def test_gerar_texto_secrets_so_com_email_nao_inclui_secao_do_firebase():
    ...
    _com_caminhos_isolados(testar, conteudo_firebase=None, conteudo_email=email_falso)
